...
import com.example.myworkbackend.repositories.*;
...
@Service
@Slf4j
public class SalaryService2 {

    private final Salary2Repository salaryRepository;
    private final Day2Repository dayRepository;
    private final Account2Repository accountRepository;

    public List<Salary2> createOrUpdateAllSalaries(LocalDate date) {
        List<Account2> accounts = accountRepository.findAll();
        List<Salary2> salaries = new ArrayList<>();
        ...
        if (LocalDate.now().isBefore(date.plusMonths(1).withDayOfMonth(1).plusDays(1))) {
            for (Account2 account : accounts) {
                Salary2 salary = createOrUpdateSalary(date, account.getId());
                salaries.add(salary);
            }
        }
        else {
    ...
    public List<Salary2> createOrUpdateAllSalariesInCase(LocalDate date) {
        List<Account2> accounts = accountRepository.findAll();
        List<Salary2> salaries = new ArrayList<>();
    ...
    public Salary2 createOrUpdateSalary(LocalDate date, String accountId) {
        List<Salary2> existingSalaries = salaryRepository.findAllByAccountId(accountId);
        List<Salary2> salariesForMonth = existingSalaries.stream()
                .filter(salary -> salary.getDate().getMonth() == date.getMonth())
                .toList();

        Account2 account = accountRepository.findById(accountId).orElse(null);
        System.out.println("??????????????????????????????? " + account.getUsername() + " " + account.getRole());

        if (!salariesForMonth.isEmpty()) {
            Salary2 existingSalary = salariesForMonth.get(0);
            ...
            assert account != null;
            System.out.println("################################ " + account.getUsername() + " " + account.getRole());
            Salary2 newSalary = Salary2.builder()
                    .date(date)
                    .accountId(accountId)
    ...
    private double calculateTotalSalary(LocalDate date, String accountId) {
        ...
        List<Day2> daysInMonth = dayRepository.findByDateBetween(
...
//                        );
    ...
    public List<Salary2> getAllSalariesForMonth(LocalDate inputDate) {
